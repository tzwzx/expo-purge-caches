...
- **Dry-run.** Run `"$PATH_VERIFY/cli" --cwd "$PROJ" --tmp "$FAKE_TMP" --name local-dry -- --dry-run -y`. `exit.txt` is `0`. `stdout.txt` contains `[dry-run] would remove: .expo` (project-local paths are relative) and `metro-cache`. After the command, `$PROJ/.expo`, `$FAKE_TMP/metro-cache`, `$FAKE_TMP/bunx-501-eas-cli@latest`, and `$FAKE_TMP/unrelated-keep-me` still exist. `stdout.txt` contains `dry-run: nothing was deleted`.
